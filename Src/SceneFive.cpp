...
void SceneFive::init(float width, float height)
{
	name = 5;
	...
	setup_vertex();
	setup_lighting();
	shader->init("D:/Uni Work/RealTimeGraphics/A1_SURNAME_FIRSTNAME/menger-sponge/Src/basic.vert", "D:/Uni Work/RealTimeGraphics/A1_SURNAME_FIRSTNAME/menger-sponge/Src/basic.frag", "D:/Uni Work/RealTimeGraphics/A1_SURNAME_FIRSTNAME/menger-sponge/Src/five.geom");
...
void SceneFive::setup_vertex()
{
	for (int i = 0; i < cube->vertex.size(); i++)
	{
		glm::vec3 begin = cube->vertex.at(i);
		glm::vec3 end = calculate_morph(begin);
		morphed.push_back(lerp(begin, end, animation));
	}
	glGenBuffers(1, &VBO);
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * cube->vertex.size(), &morphed[0], GL_STATIC_DRAW);
...
glm::vec3 SceneFive::lerp(glm::vec3 begin, glm::vec3 end, float animation)
{
...
void SceneFive::update_morphed()
{
	for (int i = 0; i < cube->vertex.size(); i++)
	{
		glm::vec3 begin = cube->vertex.at(i);
		glm::vec3 end = calculate_morph(begin);
		morphed[i] = lerp(begin, end, animation);
	}
...
void SceneFive::update_animation()
{
